...
__forceinline vec3f_t vec3f_add(vec3f_t a, vec3f_t b) {
	return (vec3f_t) { .x = a.x + b.x, .y = a.y + b.y, .z = a.z + b.z };
}
...
__forceinline vec3f_t vec3f_min(vec3f_t a, vec3f_t b) {
	return (vec3f_t) { .x = __min(a.x, b.x), .y = __min(a.y, b.y), .z = __min(a.z, b.z) };
}

__forceinline vec3f_t vec3f_max(vec3f_t a, vec3f_t b) {
	return (vec3f_t) { .x = __max(a.x, b.x), .y = __max(a.y, b.y), .z = __max(a.z, b.z) };
}

__forceinline vec3f_t vec3f_scale(vec3f_t v, float f) {
	return (vec3f_t) { .x = v.x * f, .y = v.y * f, .z = v.z * f };
}

__forceinline vec3f_t vec3f_lerp(vec3f_t a, vec3f_t b, float f) {
	return (vec3f_t) { .x = lerpf(a.x, b.x, f), .y = lerpf(a.y, b.y, f), .z = lerpf(a.z, b.z, f) };
}
...
__forceinline float vec3f_dot(vec3f_t a, vec3f_t b) {
	return (a.x * a.x) + (b.y * b.y) + (b.z * b.z);
}

__forceinline vec3f_t vec3f_reflection(vec3f_t v, vec3f_t n) {
