...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event)
{
    ...
    case SDL_WINDOWEVENT:
        ...
        if (window_event == SDL_WINDOWEVENT_CLOSE || window_event == SDL_WINDOWEVENT_MOVED || window_event == SDL_WINDOWEVENT_RESIZED)
            viewport = igFindViewportByPlatformHandle((void*)SDL_GetWindowFromID(event->window.windowID));
            if (viewport != NULL)
            {
...
static void ImGui_ImplSDL2_UpdateMousePosAndButtons()
{
    ...
    if (io->ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
    {
        ...
        if (focused_window) {
            if (igFindViewportByPlatformHandle((void*)focused_window) != NULL) {
                io->MousePos = (ImVec2){
                    .x = (float) mouse_x_global,
                    .y = (float) mouse_y_global
                };
            }
        }
    }
    else
    {
        ...
        if (SDL_GetWindowFlags(g_Window) & SDL_WINDOW_INPUT_FOCUS)
        {
            ...
            io->MousePos = (ImVec2){
                    .x = (float)(mouse_x_global - window_x),
                    .y = (float)(mouse_y_global - window_y)
            };
        }
    }
